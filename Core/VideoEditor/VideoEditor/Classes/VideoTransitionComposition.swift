...
/// 转场组合
public class VideoTransitionComposition: NSObject {
    private var composition: AVMutableComposition!
    private var videoComposition: AVMutableVideoComposition!
    private var audioMix: AVMutableAudioMix!
    private var items: [VideoTransitionItem] = []
...
// MARK: Public API
extension VideoTransitionComposition {
    public func add(asset: AVAsset, timeRange: CMTimeRange, completionHanlder handler: ((VideoTransitionItem) -> Void)? = nil) {
        ...
        let item = VideoTransitionItem.init(asset: from, to: to)
        items.append(item)
        handler?(item)
    }
    ...
    public func buildTransitionComposition() {
        ...
        for i in 0..<clipsCount {
            ...
            var item: VideoTransitionItem? = nil
            if i > 0 {
                item = items[i - 1]
            }
        ...
        for i in 0..<clipsCount {
            ...
            if i + 1 < clipsCount {
                var type: VideoTransitionItem.Transition = .dissolve
                if i < items.count {
                    type = items[i].type
                }
